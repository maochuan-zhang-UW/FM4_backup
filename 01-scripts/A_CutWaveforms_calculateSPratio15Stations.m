...
fields = {'01A', '02A', '03A', '04A', '05A', '06A', '07A','08A','09A','10A', '11A', '12A', '13A', '14A', '15A'};
% Load data
load('/Users/mczhang/Documents/GitHub/FM4/02-data/A_ID/A_Wave2F_15station.mat');
...
if ~exist('A_CutWaveforms_calculateSPratio15Stations.m', 'file')
...
addAttachedFiles(pool, '/Users/mczhang/Documents/GitHub/FM4/01-scripts/A_CutWaveforms_calculateSPratio15Stations.m');
...
for kz = 1:length(fields)
    parfor i = 1:length(Felix)
        ...
        if ~isempty(Felix(i).(['W_', fields{kz}]))
            disp(['Felix(', num2str(i), ').W_', fields{kz}, ' is not empty, skipping...']);
            continue;
        end
        ...
        if isempty(Felix(i).(['NSP_', fields{kz}])) || all(Felix(i).(['NSP_', fields{kz}]) == [0, 0, 0])
            ...
            if S_pick > 0
                ...
                try
                    [trace_Z, trace_N, trace_E] = obtain_waveforms_all15OBSs(Felix(i), kz, P.a.sttime, P.a.edtime, p);
        ...
        if ~isempty(trace_Z)
            try
                ...
                figure;plot([-0.25:0.005:1-0.005],trace_Z.dataFilt(idx_start:idx_end-1, P.filt))
            catch ME
...
%% Save output
save_path = '/Users/mczhang/Documents/GitHub/FM4/02-data/A_ID/A_Wave2F_15station.mat';
