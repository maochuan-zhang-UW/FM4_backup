%clc;
clear;close all;
path='/Users/mczhang/Documents/GitHub/FM4/02-data/';
%\fields={'AS1','AS2','CC1','EC1','EC2','EC3','ID1'};
fields = {'01A', '02A', '03A', '04A', '05A', '06A', '07A','08A','09A','10A', '11A', '12A', '13A', '14A'};

groups = {'15OBS'};
nz=1;
resultman=[];
for gp=1
    load([path,'A_ID/A_Wave2F_15station.mat']);
    for kz=1:length(fields)
        load([path,'C_SVD/C_15OBS_',fields{kz},'.mat']);
        load([path,'D_man/D_',fields{kz},'.mat']);
        for i = 1:length(wave)
            % Check if wave.Po_fields{kz} is not 0
            if wave(i).(['Po_' fields{kz}]) ~= 0 | ~isempty(wave(i).(['Po_' fields{kz}]))
                % Find the index of wave.ID in SVD_result
                index = find(SVD_result(:,1) == wave(i).ID);
                % If wave.ID is found in SVD_result, assign wave.Po_fields{kz} to SVD_result(index,6)
                if ~isempty(index)
                    SVD_result(index,7) = wave(i).(['Po_' fields{kz}]);
                end
            end
        end
        % compare with the manual pick and determine the final polarity
        n=1;
        for i = 1:length(wave)
            if wave(i).(['Po_' fields{kz}]) ~= 0 | ~isempty(wave(i).(['Po_' fields{kz}]))
                ind2 = find(SVD_result(:,1) == wave(i).ID); % this is the row of matrixs
                if isempty(ind2) | abs(wave(i).(['Po_' fields{kz}])) < 1
                    continue;
                end
                wave_test(n).ID=wave(i).ID;
                wave_test(n).Po=sign(wave(i).(['Po_' fields{kz}]));
                %            wave_test(n).wave=wave(i).(['W_',fields{kz}]);
                wave_test(n).SVD=SVD_result(ind2,2);
                mo(i) = sign(wave(i).(['Po_' fields{kz}])) * SVD_result(ind2,2);
                de(i) = abs(mo(i));
                n=n+1;
            end
        end

        WTcatpol = sum(mo) / sum(de);
        for i=1:length(wave_test)
            wave_test(i).CC_Po=sign(wave_test(i).SVD*WTcatpol);
            wave_test(i).WTcat=WTcatpol;
        end
        ind_un=find([wave_test.CC_Po]==[wave_test.Po]);
        resultman(nz,1)=nz;
        resultman(nz,2)=WTcatpol;
        resultman(nz,3)=length(ind_un)/length(wave_test);
        resultman(nz,4)=length(ind_un);
        resultman(nz,5)=length(wave_test);
        nz=nz+1;


        disp([groups{gp},'_',fields{kz},':',num2str(WTcatpol,3),', Same Ratio:' num2str(length(ind_un)/length(wave_test),2),',N:' ...
            num2str(length(ind_un)),'/',num2str(length(wave_test)),', Value: ' num2str(sum(de),2), ', out of: ' num2str(sum(abs(SVD_result(:,2))),2) ...
            ', percent: ' num2str(sum(de)/sum(abs(SVD_result(:,2)))*100,4), ',CC matched:' num2str(sum(SVD_result(:,4))/(sum(SVD_result(:,4))+sum(SVD_result(:,5))),2)]);
        %%
        % % Define the file path
        % filePath = [path,'D_man/D_',groups{gp},'_',fields{kz},'_uncon.mat'];
        % % Check if the file exists to avoid errors
        % if exist(filePath, 'file') == 2
        %     % Delete the file
        %     delete(filePath);
        %     %disp(['File ', filePath, ' deleted successfully.']);
        % else
        %     disp(['File ', filePath, ' does not exist.']);
        % end
        %%
        %save([path,'D_man/D_',groups{gp},'_',fields{kz},'_uncon.mat'], 'wave_test');clear wave_test;
        SVD_result(:,8) = sign(SVD_result(:,3) * sign(WTcatpol));    % Loop over each element in the wave structure
        % remove less than threshold
        SVD_result(find(abs(SVD_result(:,2))<10^SVD_result(1,6)),8)=0;
        % compare with the manual pick and determine the final polarity

        for j = 1:length(SVD_result)
            ind3 = find([Felix.ID]==SVD_result(j,1)); % this is the row of matrixs
            if ~isempty(ind3)
                Felix(ind3).(['Po_', fields{kz}])=SVD_result(j,8) ;

            end
        end
        clear mo de;
    end

    for i=1:length(Felix)
        po_values = [];
        for kz = 1:length(fields)
            fname = ['Po_', fields{kz}];
            if isfield(Felix(i), fname)
                value = Felix(i).(fname);
                if ~isempty(value) && isnumeric(value) && ~isnan(value)
                    po_values(end+1) = abs(value);
                end
            end
        end
        Felix(i).PoALL = sum(po_values);
        for kz=1:length(fields)
            if eval(strcat('isempty(Felix(i).Po_',fields{kz},')'))
                eval(strcat('Felix(i).Po_',fields{kz},'=0;'));
            end
            % if eval(strcat('isempty(Felix(i).SP_',fields{kz},')'))
            %     eval(strcat('Felix(i).SP_',fields{kz},'=0;'));
            % end
            if eval(strcat('isempty(Felix(i).NSP_',fields{kz},')'))
                eval(strcat('Felix(i).NSP_',fields{kz},'=[1,1,1];'));
            end
        end
    end
    save([path,'E_Po/E_Po_15OBS.mat'], 'Felix');
end



