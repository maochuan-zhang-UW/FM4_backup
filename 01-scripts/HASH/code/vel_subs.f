...
* Examine table
      i = 0
      do dep = dep1,dep2+dep3/20,dep3
        i=i+1
        j=0
        do x = del1,del2+del3/20,dep3
          j=j+1
*      print* '',dep,x,i,j,table(j,i,1)
        end do
      end do
...
c check if outside depth range
     ...
     &           event depth=',qdep,' table range=',d(1),d(nd)
         return
      end if
