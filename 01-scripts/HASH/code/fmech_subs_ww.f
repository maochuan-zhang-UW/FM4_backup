...
c
* Modified by William Wilcock (9/2007) return a random sampling
* of mechanisms resulting form successful trials rather than a random
* sampling of mechanisms that are successful in at least 1 trial 
c
...
* WSDW start mod ____________ - changed dimensioning and eliminated irotgood2
      integer irotgood(ncoor*nmax0)
* WSDW end mod   ____________
      real fran
...
* WSDW start mod ____________ - Unnecessary
*      do irot=1,nrot
*        irotgood(irot)=0
*      end do
* WSDW end mod   ____________
...
* WSDW start mod ____________ - Take nfault=0 outside trial loop (im)
      nfault = 0
* WSDW end mod   ____________
      do 430 im=1,nmc 
        print*,'nmc im ',nmc,im
...
c choose fit criteria
         ...
         print*,'nmiss0max, nmissmax ',nmiss0max,nmissmax
...
* WSDW start mod ____________ - traking every good trial not every mechanism
*                               with at least 1 good trial
              nfault = nfault+1
              irotgood(nfault)=irot
* WSDW start mod ____________ - changed dimensioning
...
c  Select output solutions  
        ...
        if (nfault.le.maxout) then
          do i=1,nfault
* WSDW start mod ____________ - irotgood2 to irotgood
            irot=irotgood(i)
            nf=nf+1
