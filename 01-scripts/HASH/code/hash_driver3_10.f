...
*      integer npolmin                                  ! minimum number of observations WSDW not used so eliminate
      ...
      integer minextra                                 ! WSDW minimum value of nextra
* wsdw end _________________________
...
* wsdw start _________________________
      character*100 outfile2,outfile3
* wsdw end _________________________
...
* wsdw start _________________________
      print *,'Number of additional polarity misfits above best fit'
      read *,minextra
* wsdw end _________________________
...
* wsdw start _______________________________________
*      nmismax=max(nint(nppl*badfrac),2)                    
*      nextra=max(nint(nppl*badfrac*0.5),2)
      nmismax=max(nint(nppl*badfrac),0)                    
      nextra=max(nint(nppl*badfrac*0.5),minextra)
      print*,'nmismax, nextra ',nmismax,nextra
*      qmismax=max(nspr*qbadfac,2.0)                    
*      qextra=max(nspr*qbadfac*0.5,2.0)
      qmismax=max(nspr*qbadfac,0.0)                    
      qextra=max(nspr*qbadfac*0.5,0.0)
* wsdw end _______________________________________
...
*@ TRYING THIS>
     ...
     &       dang2,maxout,nextra,nmismax,nf2,strike2,dip2,
     &       rake2,f1norm,f2norm)
      print*,'nf2 ',nf2
