...
% data_path = [BASE_PATH, 'FM3/02-data/'];
...
for gp = 1:length(groups) % Process all groups
    % Load and filter data
    %load([BASE_PATH, 'FM/02-data/D_Po_', groups{gp}, '.mat']);
    load('/Users/mczhang/Documents/GitHub/FM4/02-data/E_Po/E_Po_rightA01.mat');
    %Felix=Felix_combined;clear Felix_combined;
    Po = Felix([Felix.PoALL] > min_Po_num);
    %  % Filter events for this period
    % if gp == 1
    %     % "Before": events earlier than date_BF
    %     indEv = find([Po.on] < date_BF);
    % elseif gp == 2
    %     % "During": events between date_BF and date_DR
    %     indEv = find([Po.on] >= date_BF & [Po.on] < date_DR);
    % else
    %     % "After": events on/after date_DR
    %     indEv = find([Po.on] >= date_DR);
    % end
    % Po = Po(indEv);
    ...
    for kz = 1:n_fields
        ...
        %sp_ratios(:,kz) = vertcat(Po.(['NSP_' fields{kz}]));
        % Extract NSP values and compute log10(NSP(3)/NSP(1))
        NSP_vals = vertcat(Po.(['NSP_' fields{kz}]));  % 9678×3 matrix
        sp_ratios(:,kz) = log10(NSP_vals(:,3) ./ NSP_vals(:,1));
    end
 ...
 % If Po does not have a field named 'Cluster', add it
    ...
    %%
    save(['/Users/mczhang/Documents/GitHub/FM3/02-data/F_Cl/FA_Cl_' groups{gp} '_new_Cluster_Result67.mat'], "Po_Clu");
end
