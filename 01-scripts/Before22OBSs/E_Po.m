...
for gp=7%:length(groups)
    ...
    for kz=1:length(fields)
        ...
        load([path_old,'D_man/D_',groups{gp},'_',fields{kz},'.mat']);
        for i = 1:length(wave)
            % Check if wave.Po_fields{kz} is not 0
            if wave(i).([ fields{kz},'_Po']) ~= 0 | ~isempty(wave(i).([fields{kz},'_Po']))
                ...
                if ~isempty(index)
                    SVD_result(index,7) = wave(i).([ fields{kz},'_Po']);
                end
            end
        end
        ...
        for i = 1:length(wave)
            if wave(i).([ fields{kz},'_Po']) ~= 0 | ~isempty(wave(i).([fields{kz},'_Po']))
                ...
                if isempty(ind2) | abs(wave(i).([ fields{kz},'_Po'])) < 1
                    continue;
                end
                wave_test(n).ID=wave(i).ID;
                wave_test(n).Po=sign(wave(i).([ fields{kz},'_Po']));
                %            wave_test(n).wave=wave(i).(['W_',fields{kz}]);
                wave_test(n).SVD=SVD_result(ind2,2);
                mo(i) = sign(wave(i).([ fields{kz},'_Po'])) * SVD_result(ind2,2);
                de(i) = abs(mo(i));
                n=n+1;
            end
        end
    ...
    for i=1:length(Felix)
        Felix(i).PoALL=sum([abs(Felix(i).Po_AS1),abs(Felix(i).Po_AS2),abs(Felix(i).Po_CC1),abs(Felix(i).Po_EC3),abs(Felix(i).Po_EC2),abs(Felix(i).Po_EC1),abs(Felix(i).Po_ID1)],'omitnan');
        for kz=1:length(fields)
    ...
    save([path,'E_Po/E_',groups{gp},'.mat'], 'Felix');
end
